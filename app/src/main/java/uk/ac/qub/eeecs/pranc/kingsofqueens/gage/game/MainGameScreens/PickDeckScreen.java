...
public class PickDeckScreen extends GameScreen {
    ...
    public void updateTouchEvents(List<TouchEvent> touchEvents) {
        if (!touchEvents.isEmpty()) {
            ...
            if(genAlgorithm.hasTouchEventType0(touchEvent,DeckButton)){
                mangeDeckSelection();
            }
            ...
            if (genAlgorithm.hasTouchEventType0(touchEvent,Left)) {
                index--;
            }
            if (genAlgorithm.hasTouchEventType0(touchEvent,Right)) {
                index++;
            }
    ...
    private void checkPlayButtonPressed(TouchEvent touchEvent) {
        try {
            if (deck1Picked && deck2Picked) {
                if (genAlgorithm.hasTouchEventType0(touchEvent,Play)) {
    ...
    private boolean checkInputDeckChoices(TouchEvent touchEvent, DeckPickerRect deck, boolean valid) {
        if (valid == true) {
            if (genAlgorithm.hasTouchEventType0(touchEvent,deck.getButton())){
                valid = false;
                deck.setDeckName("");
            }
        }
